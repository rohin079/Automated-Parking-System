...
def enter_third():
    ...
    # video
    cv2.destroyAllWindows()
    cap = cv2.VideoCapture('VN20221021_095309.mp4') #<-------------------------------------video source
    def video_stream():
        cv2image = cv2.cvtColor(cap.read()[1], cv2.COLOR_BGR2RGB)
        pl = detect.plate_read(cv2image[350: 600,350:1000])
        if not pl:
            if (l != []):
                ml = (statistics.mode(l))
                s = detect.googlesheet(ml,client,detect.time())
                print("Entered in google sheet")
                trv.insert("", 'end', text=dt[0],values =(s[0],s[1],s[2]))# dt[2],dt[3]))
                detect.sendgmail(s,client)
                print("Mail sent")
                detect.sendwhatsapp(s,client)
    ...
    def enter_roi():
        ...
        # Capture from camera
        cap = cv2.VideoCapture('VN20221021_095309.mp4')
    ...
    path = 'gs_credentials.json'
    gc=pygsheets.authorize(service_account_file=path)
    sheetname = 'Parking slots'
    ...
    """trv.heading("1", text ="PlateNumber")
    trv.heading("2", text ="Slot")"""
